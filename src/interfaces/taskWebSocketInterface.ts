...
export class TaskWebSocketHandler {
  ...
  private handleUpdateTask(payload: {
    taskId: string;
  ...
  private handleRemoveTask(payload: { taskId: string }): {
    success: boolean;
    message?: string;
  } {
    if (!payload?.taskId) {
    ...
    return {
      success,
      message: success
        ? `Task ${payload.taskId} removed successfully`
        : `Task ${payload.taskId} not found`,
    };
  }

  private handleGetTask(payload: {
    taskId: string;
  }): { task: CognitiveItem } | { task: null; message: string } {
    if (!payload?.taskId) {
      throw new Error('Missing required field: taskId');
    }
  ...
  private handleUpdateTaskStatus(payload: {
    taskId: string;
    status: TaskStatus;
  }): { task: CognitiveItem } {
