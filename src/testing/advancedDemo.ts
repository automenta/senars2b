...
async function runAdvancedDemo() {
  ...
  try {
    ...
    if (beliefs.length > 0) {
      ...
      beliefs.forEach((item, index) => {
        ...
        if (item.truth) {
          console.log(
            `     Truth: Frequency=${item.truth.frequency.toFixed(2)}, Confidence=${item.truth.confidence.toFixed(2)}`
          );
        }
      });
    }
  ...
  console.log(
    'and extract multiple types of cognitive items from natural language input.'
  );
}
