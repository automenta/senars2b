...
async function runBenchmark() {
  ...
  // Add 100 beliefs
  for (let i = 0; i < 100; i++) {
    await core.addInitialBelief(`Test belief ${i}`, truth, attention, {
      domain: 'test',
      author: 'benchmark',
      trust_score: 0.8,
    });
  ...
  // Add 50 goals
  for (let i = 0; i < 50; i++) {
    await core.addInitialGoal(`Test goal ${i}`, attention, {
      domain: 'test',
      author: 'benchmark',
    });
  }
  ...
  for (const [workerId, stats] of status.workerStats.entries()) {
    console.log(
      `    Worker ${workerId}: ${stats.itemsProcessed} items processed, ${stats.errors} errors`
    );
  }
