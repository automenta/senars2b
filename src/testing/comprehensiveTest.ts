...
async function runComprehensiveTest() {
  ...
  console.log(
    `  World Model - Atoms: ${status.worldModelStats.atomCount}, Items: ${status.worldModelStats.itemCount}`
  );
  ...
  const complexGoalAttention: AttentionValue = {
    priority: 0.95,
    durability: 0.9,
  };
  await core.addInitialGoal(
    'Create a comprehensive health plan for my pet including diet, exercise, and medical checkups',
    complexGoalAttention,
    {
      domain: 'veterinary',
      source: 'user',
      trust_score: 0.9,
  ...
  for (let i = 0; i < 5; i++) {
    await core.addInitialBelief(
      `Related fact ${i + 1} about pet health`,
      mediumTruth,
      lowAttention,
      {
        domain: 'veterinary',
