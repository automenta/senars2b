...
async function runComprehensiveTest() {
    ...
    await core.addInitialBelief("Cats are curious animals", mediumTruth, mediumAttention, {
        domain: "behavioral",
    ...
    await core.addInitialGoal("Diagnose why my cat is sick", highAttention, {
        domain: "veterinary",
        source: "user",
        trust_score: 0.9
    });
    ...
    await core.addInitialBelief("Chocolate is safe for cats", conflictingTruth, mediumAttention, {
    ...
    await core.addInitialGoal("Create a comprehensive health plan for my pet including diet, exercise, and medical checkups", complexGoalAttention, {
        domain: "veterinary",
        source: "user",
        trust_score: 0.9
    });
    ...
    for (let i = 0; i < 5; i++) {
        await core.addInitialBelief(`Related fact ${i + 1} about pet health`, mediumTruth, lowAttention, {
            domain: "veterinary",
            source: "vetdb.org",
            trust_score: 0.85
        });
