...
async function testBeliefRevisionEngine(): Promise<void> {
  ...
  console.log(
    `Merged truth - Frequency: ${mergedTruth.frequency.toFixed(2)}, Confidence: ${mergedTruth.confidence.toFixed(2)}`
  );
...
async function testAttentionModule(): Promise<void> {
  ...
  console.log(
    `Initial attention - Priority: ${initialAttention.priority.toFixed(2)}, Durability: ${initialAttention.durability.toFixed(2)}`
  );
}
