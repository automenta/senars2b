...
import { FaPlay, FaPause, FaStop, FaCheck, FaEdit, FaTrash } from 'react-icons/fa';
...
const TaskControls: React.FC<TaskControlsProps> = ({ task, sendMessage }) => {
  ...
  return (
    <div className={styles.controls}>
      ...
      <button onClick={handleEdit} title="Edit" aria-label={`Edit task ${task.title}`} className={`${styles.button} ${styles.editBtn}`}><FaEdit /></button>
      <button onClick={handleDelete} title="Delete" aria-label={`Delete task ${task.title}`} className={`${styles.button} ${styles.deleteBtn}`}><FaTrash /></button>
    </div>
  );
};
