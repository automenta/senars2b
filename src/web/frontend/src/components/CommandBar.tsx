...
const CommandBar: React.FC = () => {
  ...
  const [cliHistory, setCliHistory] = useState<string[]>([
    'Welcome to the Senars3 Web CLI!',
  ]);
    ...
    if (e.key === 'Enter') {
      ...
      switch (cmd) {
        ...
        case 'help':
          setCliHistory((prev) => [
            ...prev,
            'Available commands: status, stats, process <input>, clear, help',
          ]);
          break;
        default:
          if (cmd.includes('.')) {
        ...
        <div className={styles.resultContent}>
          {result.cognitiveItems.map((item: CognitiveItem) => (
            <div key={item.id} className={styles.cognitiveItem}>
              ...
              {item.truth && (
                <div>
                  <strong>Truth:</strong> F={item.truth.frequency}, C=
                  {item.truth.confidence}
                </div>
              )}
              {item.attention && (
                <div>
                  <strong>Attention:</strong> P={item.attention.priority}, D=
