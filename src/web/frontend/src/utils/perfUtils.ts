...
export const perfUtils = {
  ...
  observeComponentRender: (componentName: string): PerformanceObserver => {
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        console.log(
          `${componentName} rendered in ${entry.duration} milliseconds`
        );
      }
    });
  ...
  logMemoryUsage: (): void => {
    ...
    if (memory) {
      console.log(
        `Memory usage: ${Math.round(memory.usedJSHeapSize / 1048576)} MB used out of ${Math.round(memory.jsHeapSizeLimit / 1048576)} MB available`
      );
    }
  },
};
