...
const ConfigurationView: React.FC = () => {
  const { theme, toggleTheme, notificationsEnabled, toggleNotifications } =
    useStore();
  const [attentionParams, setAttentionParams] = useState<AttentionParams>({
    priorityWeight: 0.7,
    durabilityWeight: 0.3,
  });
  const [reflectionParams, setReflectionParams] = useState<ReflectionParams>({
    enabled: true,
    someParameter: 0.5,
  });
  ...
  useEffect(() => {
    ...
    sendMessage({
      ...
      payload: {},
      id: 'attention.getStatistics',
    });
    sendMessage({
      target: 'reflection',
      method: 'getStatistics',
      payload: {},
      id: 'reflection.getStatistics',
    });
  }, [sendMessage]);
  ...
  const handleSave = () => {
    sendMessage({
      ...
      payload: attentionParams,
    });
    sendMessage({
      target: 'reflection',
      method: 'setParameters',
      payload: {
        enabled: reflectionParams.enabled,
        parameters: { someParameter: reflectionParams.someParameter },
      },
    });
    sendMessage({
      target: 'reflection',
      method: 'setEnabled',
      payload: { enabled: reflectionParams.enabled },
    });
  };
        ...
        <div className={styles.cardBody}>
          <div className={styles.settingRow}>
            ...
            <div className={styles.themeSwitcher}>
              <button onClick={toggleTheme} disabled={theme === 'light'}>
                Light
              </button>
              <button onClick={toggleTheme} disabled={theme === 'dark'}>
                Dark
              </button>
            </div>
          </div>
        ...
        <div className={styles.cardBody}>
          <div className={styles.settingRow}>
            ...
            <input
              type="number"
              name="priorityWeight"
              value={attentionParams.priorityWeight}
          ...
          <div className={styles.settingRow}>
            ...
            <input
              type="number"
              name="durabilityWeight"
              value={attentionParams.durabilityWeight}
              onChange={handleAttentionChange}
              step="0.1"
              min="0"
              max="1"
          ...
          {attentionStats && (
            <pre>{JSON.stringify(attentionStats, null, 2)}</pre>
          )}
        </div>
      </div>
        ...
        <div className={styles.cardBody}>
          <div className={styles.settingRow}>
            ...
            <input
              type="checkbox"
              name="enabled"
              checked={reflectionParams.enabled}
          ...
          <div className={styles.settingRow}>
            ...
            <input
              type="number"
              name="someParameter"
              value={reflectionParams.someParameter}
              onChange={handleReflectionChange}
              step="0.1"
              min="0"
              max="1"
          ...
          {reflectionStats && (
            <pre>{JSON.stringify(reflectionStats, null, 2)}</pre>
          )}
        </div>
      </div>
