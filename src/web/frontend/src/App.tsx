...
function App() {
        ...
        switch (message.type) {
          case 'TASK_LIST_UPDATE': {
            ...
            if (tempId) {
              setTasks(
                tasks.map((task) =>
                  task.id === tempId
                    ? newTasks.find((t: Task) => t.title === task.title) || task
                    : task
                )
              );
          ...
          case 'TASK_UPDATE':
            addToastNotification(
              `Task "${message.payload.title}" updated.`,
              'info'
            );
            break;
          case 'TASK_DELETED':
          ...
          case 'PROMPT_NEW':
            ...
            addToastNotification(
              'You have a new prompt in your inbox!',
              'info'
            );
            break;
          case 'NOTIFICATION':
  ...
  const { isConnected, connectionError, sendMessage } =
    ...
    <ErrorBoundary>
      <div className="app-container">
        <main className="main-content">
          ...
          />
          <TasksView sendMessage={sendMessage} onAddTask={handleAddTask} />
        </main>
        <CommandBar />
      </div>
    </ErrorBoundary>
