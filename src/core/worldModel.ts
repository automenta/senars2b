...
import {
  ...
  SimpleBeliefRevisionEngine,
} from './beliefRevisionEngine';
import { CognitiveItemFactory } from '../modules/cognitiveItemFactory';
import logger from '../services/logger';
import {
  ANALOGY_HYPOTHESIS_DURABILITY,
  ANALOGY_HYPOTHESIS_PRIORITY,
  BELIEF_UPDATE_EVENT_DURABILITY,
  BELIEF_UPDATE_EVENT_PRIORITY,
  CAUSAL_INFERENCE_CONFIDENCE,
  CAUSAL_INFERENCE_DURABILITY,
  CAUSAL_INFERENCE_FREQUENCY,
  CAUSAL_INFERENCE_PRIORITY,
  COMPACTION_DURABILITY_THRESHOLD,
  CONFIDENCE_DISTRIBUTION_BIN_COUNT,
  DEFAULT_QUERY_K,
  DEFAULT_SCHEMA_DURABILITY,
  DEFAULT_SCHEMA_PRIORITY,
  ONE_DAY_IN_MS,
  STRUCTURAL_SIMILARITY_KEY_WEIGHT,
  STRUCTURAL_SIMILARITY_VALUE_WEIGHT,
...
export interface WorldModel {
  ...
  ): [CognitiveItem | null, CognitiveItem | null];
...
export class PersistentWorldModel implements WorldModel {
  ...
  query_by_semantic(embedding: number[], k: number): CognitiveItem[] {
    ...
    const similarities = Array.from(this.semanticIndex.entries()).map(
      ([atomId, atomEmbedding]) => {
    ...
    return this._getItemsByAtomIds(topAtoms);
  }

  query_by_symbolic(pattern: any, k: number = DEFAULT_QUERY_K): CognitiveItem[] {
    // Pattern matching implementation using the index
    const patternStr =
      typeof pattern === 'string'
        ? pattern
        : JSON.stringify(pattern).toLowerCase();

    // Score atoms based on symbolic match
    const scoredAtoms = Array.from(this.symbolicIndex.entries()).map(
      ([atomId, content]) => {
        const score = this.calculateStringOverlap(
          patternStr,
          content.toLowerCase()
        );
        return { atomId, score };
  ...
  private _getItemsByAtomIds(atomIds: string[]): CognitiveItem[] {
    return atomIds
      .map((atomId) => {
        // Find items that reference this atom
        return Array.from(this.items.values()).find(
          (item) => item.atom_id === atomId
        );
      })
  ...
  revise_belief(
    ...
    if (existing && existing.truth) {
      ...
      // Check for conflict
      if (
        this.beliefRevisionEngine.detect_conflict(
          existing.truth,
          new_item.truth
        )
      ) {
        ...
        revisedItem = this.beliefRevisionEngine.resolve_conflict(
          existing,
          new_item
        );
      } else {
      ...
      event = CognitiveItemFactory.createEvent(
        ...
        {
          priority: BELIEF_UPDATE_EVENT_PRIORITY,
          durability: BELIEF_UPDATE_EVENT_DURABILITY,
        } // System-level event attention
      );
  ...
  private fallbackSchemaGenerators: Record<
    string,
    (
      atom: SemanticAtom
    ) => (a: CognitiveItem, b: CognitiveItem, worldModel: WorldModel) => CognitiveItem[]
  > = {
    EnhancedAnalogyHypothesis: (atom) => (a, b, _) => {
      const newItem = {
        id: uuidv4(),
        atom_id: atom.id,
        type: 'QUERY' as const,
        attention: {
          priority: ANALOGY_HYPOTHESIS_PRIORITY,
          durability: ANALOGY_HYPOTHESIS_DURABILITY,
        },
        stamp: {
          timestamp: Date.now(),
    ...
    EnhancedCausalInference: (atom) => (a, b, _) => {
      const newItem = {
        id: uuidv4(),
        atom_id: atom.id,
        type: 'BELIEF' as const,
        truth: {
          frequency: CAUSAL_INFERENCE_FREQUENCY,
          confidence: CAUSAL_INFERENCE_CONFIDENCE,
        },
        attention: {
          priority: CAUSAL_INFERENCE_PRIORITY,
          durability: CAUSAL_INFERENCE_DURABILITY,
        },
        stamp: {
          timestamp: Date.now(),
          parent_ids: [a.id, b.id],
  ...
  private _getSchemaApplyFunction(
    atom: SemanticAtom
  ): (a: CognitiveItem, b: CognitiveItem, worldModel: WorldModel) => CognitiveItem[] {
    // Case 1: The content is a well-defined schema with its own apply function
    if (
      atom.content &&
      typeof atom.content === 'object' &&
      atom.content.type === 'schema' &&
      'apply' in atom.content &&
      typeof atom.content.apply === 'function'
    ) {
      return (a, b, worldModel) => {
        try {
          return atom.content.apply(a, b, worldModel);
        } catch (error) {
          logger.warn(
            { schemaId: atom.id, error },
            `Schema apply function failed`
          );
          return [];
        }
      };
    ...
    const schemaName = (atom.content as any)?.name;
    if (typeof schemaName === 'string') {
      const fallbackGenerator = this.fallbackSchemaGenerators[schemaName];
      if (fallbackGenerator) {
        return fallbackGenerator(atom);
      }
    }
    ...
    return (a, b, _) => {
      const newItem = {
        ...
        type: 'QUERY' as const,
        attention: {
          priority: DEFAULT_SCHEMA_PRIORITY,
          durability: DEFAULT_SCHEMA_DURABILITY,
        },
        stamp: {
          timestamp: Date.now(),
          parent_ids: [a.id, b.id],
  ...
  register_schema_atom(atom: SemanticAtom): CognitiveSchema {
    const schema: CognitiveSchema = {
      atom_id: atom.id,
      apply: this._getSchemaApplyFunction(atom),
    };
  ...
  compact(): void {
    ...
    const now = Date.now();
    const oneDayAgo = now - ONE_DAY_IN_MS;

    for (const [id, item] of this.items.entries()) {
      if (
        item.attention.durability < COMPACTION_DURABILITY_THRESHOLD &&
        item.stamp.timestamp < oneDayAgo
      ) {
        this.items.delete(id);
        this.attentionIndex.delete(id);
  ...
  getConfidenceDistribution(): { bins: string[]; counts: number[] } {
    ...
    const binCount = CONFIDENCE_DISTRIBUTION_BIN_COUNT;
    ...
    for (const belief of beliefs) {
      if (belief.truth) {
        ...
        const binIndex = Math.min(
          Math.floor(confidence * binCount),
          binCount - 1
        );
        counts[binIndex]++;
      }
  ...
  getStatistics(): {
    ...
    const durabilities = Array.from(this.items.values()).map(
      (item) => item.attention.durability
    );
    const avgDurability =
  ...
  private calculateStringOverlap(pattern: string, content: string): number {
    ...
    const intersection = new Set(
      [...patternWords].filter((x) => contentWords.has(x))
    );
  ...
  private calculateStructuralSimilarity(obj: any, pattern: any): number {
    ...
    const keyMatchScore =
      commonKeys.length / Math.max(patternKeys.length, objKeys.length);

    // Calculate value matching score for common keys
    let valueMatchScore = 0;
    if (commonKeys.length > 0) {
      valueMatchScore =
        commonKeys.reduce((score, key) => {
          return (
            score + this.calculateStructuralSimilarity(obj[key], pattern[key])
          );
        }, 0) / commonKeys.length;
    }
    ...
    return (
      STRUCTURAL_SIMILARITY_KEY_WEIGHT * keyMatchScore +
      STRUCTURAL_SIMILARITY_VALUE_WEIGHT * valueMatchScore
    );
  }
}
