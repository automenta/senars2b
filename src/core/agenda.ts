...
export class PriorityAgenda implements Agenda {
  ...
  constructor(
    ...
    if (!getTaskStatus) {
      throw new Error(
        'A getTaskStatus function must be provided for robust dependency checking.'
      );
    }
        ...
        if (item.task_metadata.parent_id) {
          ...
          if (
            ...
            if (totalSubtasks > 0) {
              ...
              parentTask.task_metadata.completion_percentage =
                (completedCount / totalSubtasks) * 100;
  ...
  push(item: CognitiveItem): void {
    if (!item || !item.id || !item.attention) {
      throw new Error(
        'Item must be a valid CognitiveItem with an id and attention value.'
      );
    }
    if (
      typeof item.attention.priority !== 'number' ||
      item.attention.priority < 0 ||
      item.attention.priority > 1
    ) {
      throw new Error(
        'Item attention priority must be a number between 0 and 1.'
      );
    }
  ...
  async pop(): Promise<CognitiveItem> {
    const unblockedItemIndex = this.items.findIndex(
      (item) => !this.isBlocked(item)
    );
    ...
    return new Promise<CognitiveItem>((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(
          new Error('Timeout waiting for an unblocked item to become available')
        );
  ...
  getStatistics(): {
    ...
    const popRate =
      timeElapsedSeconds > 0 ? popsSinceLastCheck / timeElapsedSeconds : 0;
    ...
    const averageWaitTime =
      this.popCount > 0 ? this.totalWaitTime / this.popCount : 0;
  ...
  getTasksBy(filter: {
    ...
    return this.items.filter((item) => {
      ...
      if (
        filter.category &&
        (!metadata.categories || !metadata.categories.includes(filter.category))
      )
        return false;
  ...
  getTasksByGroup(groupId: string): CognitiveItem[] {
    return this.items.filter(
      (item) => item.type === 'TASK' && item.task_metadata?.group_id === groupId
    );
  }
  ...
  private getCombinedPriority(item: CognitiveItem): number {
    ...
    if (item.type === 'TASK' && item.task_metadata) {
      ...
      const taskPriority =
        priorityMap[item.task_metadata.priority_level] || 0.5;
  ...
  private sortItemsByPriority(): void {
    this.items.sort(
      (a, b) => this.getCombinedPriority(b) - this.getCombinedPriority(a)
    );
  }
