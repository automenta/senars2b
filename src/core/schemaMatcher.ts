...
export class EfficientSchemaMatcher implements SchemaMatcher {
  ...
  private buildReteNetwork(): void {
    ...
    const contentNode = new ReteNode(
      'content',
      (item: CognitiveItem, atom: any) =>
        atom &&
        (typeof atom.content === 'string' || typeof atom.content === 'object')
    );
    ...
    for (const [schemaId, schemaAtom] of this.schemaAtoms.entries()) {
      if (
        schemaAtom.content &&
        typeof schemaAtom.content === 'object' &&
        'name' in schemaAtom.content
      ) {
        const schemaNode = new ReteNode(
          `schema-${schemaAtom.content.name}`,
          () => true
        );
