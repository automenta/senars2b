...
export class HybridResonanceModule implements ResonanceModule {
  find_context(
    ...
    const semanticMatches = world_model.query_by_semantic(
      queryAtom.embedding,
      k * 2
    );
    const symbolicMatches = world_model.query_by_symbolic(
      queryAtom.content,
      k * 2
    );
    const structuralMatches = world_model.query_by_structure(
      queryAtom.content,
      k
    );
  ...
  private calculateRelevanceScore(
    queryItem: CognitiveItem,
    candidateItem: CognitiveItem,
    world_model: WorldModel
  ): number {
    ...
    const symbolicScore = this.calculateSymbolicOverlap(
      queryItem,
      candidateItem,
      world_model
    );
    ...
    const diversityScore = this.calculateDiversityFactor(
      candidateItem,
      world_model
    );
  ...
  private calculateSymbolicOverlap(
    itemA: CognitiveItem,
    itemB: CognitiveItem,
    world_model: WorldModel
  ): number {
  ...
  private calculateStructuralSimilarity(
    ...
    if (
      typeof atomA.content === 'object' &&
      typeof atomB.content === 'object'
    ) {
  ...
  private calculateGoalRelevance(
    ...
    if (
      itemA.goal_parent_id === itemB.id ||
      itemB.goal_parent_id === itemA.id
    ) {
      return 1.0;
    }

    if (
      itemA.goal_parent_id &&
      itemB.goal_parent_id &&
      itemA.goal_parent_id === itemB.goal_parent_id
    ) {
      return 0.8;
    }
  ...
  private getGoalAncestors(item: CognitiveItem): string[] {
    ...
    const currentId = item.goal_parent_id;
  ...
  private calculateRecencyFactor(
    ...
    const temporalProximity = Math.exp(
      -Math.abs(queryAge - candidateAge) / (1000 * 60 * 60 * 24)
    );
