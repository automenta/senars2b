...
import { UnifiedAgenda, AgendaConfig, PriorityWeighting } from './UnifiedAgenda';
import { Logger } from '@/utils/standardLogger';

interface PriorityAgendaConfig extends AgendaConfig {
  deadlineWindowMs?: number;
  taskPriorityWeight?: number;
  deadlineFactorWeight?: number;
  attentionPriorityWeight?: number;
  completionFactorWeight?: number;
}
 ...
 */
export class RefactoredPriorityAgenda extends UnifiedAgenda {
  ...
  constructor(
    ...
    userConfig: Partial<PriorityAgendaConfig> = {}
  ) {
    const defaultConfig: PriorityAgendaConfig = {
      maxWaitTime: 30000, // 30 seconds
      enableDependencyTracking: true,
      deadlineWindowMs: PriorityAgenda.DEFAULT_DEADLINE_WINDOW_MS,
      taskPriorityWeight: DEFAULT_WEIGHTING.taskPriority,
      deadlineFactorWeight: DEFAULT_WEIGHTING.deadlineFactor,
      attentionPriorityWeight: DEFAULT_WEIGHTING.attentionPriority,
      completionFactorWeight: DEFAULT_WEIGHTING.completionFactor
    };
    ...
    this.weighting = {
      taskPriority: this.getConfig().taskPriorityWeight || DEFAULT_WEIGHTING.taskPriority,
      deadlineFactor: this.getConfig().deadlineFactorWeight || DEFAULT_WEIGHTING.deadlineFactor,
      attentionPriority: this.getConfig().attentionPriorityWeight || DEFAULT_WEIGHTING.attentionPriority,
      completionFactor: this.getConfig().completionFactorWeight || DEFAULT_WEIGHTING.completionFactor
    };
  }
  ...
  private getCombinedPriority(item: CognitiveItem): number {
    ...
    if (item.type === 'TASK' && item.task_metadata) {
      ...
      if (item.task_metadata.deadline) {
        ...
        } else if (timeLeft < (this.getConfig().deadlineWindowMs || PriorityAgenda.DEFAULT_DEADLINE_WINDOW_MS)) {
          // Urgency increases as the deadline approaches.
          deadlineFactor = 1.0 - (timeLeft / (this.getConfig().deadlineWindowMs || PriorityAgenda.DEFAULT_DEADLINE_WINDOW_MS));
        }
      }
