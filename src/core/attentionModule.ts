...
export class DynamicAttentionModule implements AttentionModule {
  private accessHistory: Map<string, { timestamp: number; count: number }[]> =
    new Map();
  ...
  calculate_derived(
    ...
    const avgPriority =
      parents.reduce((sum, p) => sum + p.attention.priority, 0) /
      parents.length;
    const avgDurability =
      parents.reduce((sum, p) => sum + p.attention.durability, 0) /
      parents.length;
    ...
    return {
      priority: Math.min(
        1.0,
        avgPriority * trustFactor * inhibitionFactor + noveltyBonus
      ),
  ...
  update_on_access(items: CognitiveItem[]): void {
    ...
    for (const item of items) {
      ...
      if (accessCount > 3) {
        // If accessed more than 3 times recently
        item.attention.durability = Math.min(
          1.0,
          item.attention.durability + 0.02
        );
      }
    }
  }
          ...
          if (timeSinceAccess > decayThreshold) {
            ...
            if (!item) {
              const worldItem = (world_model as any).get_item
                ? (world_model as any).get_item(itemId)
                : null;
            ...
            if (item) {
              ...
              item.attention.durability = Math.max(
                0.01,
                item.attention.durability
              );
