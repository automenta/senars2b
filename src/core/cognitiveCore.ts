...
import { SystemSchemaRegistry } from './systemSchemaRegistry';
...
export class DecentralizedCognitiveCore {
   ...
   */
  constructor(
    dependencies: CognitiveCoreDependencies,
    ...
    this.statisticsManager = new StatisticsManager(this.config.workerCount);

    new SystemSchemaRegistry(this.worldModel, this.schemaMatcher).register();
  }
  ...
  async start(): Promise<void> {
    ...
    // Start worker pool
    const workers = Array(this.config.workerCount)
      .fill(null)
      .map((_, i) => this.createWorker(i));
  ...
  public async addInitialBelief(
    ...
    const validationResult = AddBeliefSchema.safeParse({
      content,
      truth,
      attention,
      meta,
    ...
    if (!validationResult.success) {
      throw new Error(
        `Invalid belief input: ${JSON.stringify(validationResult.error.flatten().fieldErrors)}`
      );
    }
   ...
   * @throws Error if any required parameters are missing or invalid
   */
  public async addInitialGoal(
    content: any,
    attention: AttentionValue,
    meta?: Record<string, any>
  ): Promise<void> {
    // Validate input using Zod schema
    const validationResult = AddGoalSchema.safeParse({
      content,
      attention,
      meta,
    });
    if (!validationResult.success) {
      throw new Error(
        `Invalid goal input: ${JSON.stringify(validationResult.error.flatten().fieldErrors)}`
      );
    }
    const { content: validatedContent } = validationResult.data;
  ...
  public async addSchema(
    ...
    if (!validationResult.success) {
      throw new Error(
        `Invalid schema input: ${JSON.stringify(validationResult.error.flatten().fieldErrors)}`
      );
    }
    const { content: validatedContent } = validationResult.data;
  ...
  public getSystemStatus(): {
    ...
    return {
      ...
      worldModelStats: this.worldModel.getStatistics(),
      workerStats: Object.fromEntries(
        this.statisticsManager.getWorkerStatistics()
      ),
      taskStats: this.taskManager.getTaskStatistics(),
      performance: this.statisticsManager.getPerformanceStats(),
      systemInfo: this.statisticsManager.getSystemInfo(),
    };
  }
  ...
  private async _createAndStoreAtom(
    content: string,
    meta: Record<string, any>
  ): Promise<SemanticAtom> {
  ...
  private async createWorker(workerId: number): Promise<void> {
    ...
    while (this.isRunning) {
      try {
        ...
        this.statisticsManager.recordWorkerError(workerId);
      }
    }
  ...
  private async processItem(
    ...
    try {
      ...
      const contextItems = this.resonanceModule.find_context(
        itemA,
        this.worldModel,
        CONTEXT_ITEM_COUNT
      );
      ...
      logger.error(
        { itemId: itemA.id, error },
        'Worker failed processing item'
      );
      throw error;
    } finally {
      const duration = Date.now() - startTime;
      this.statisticsManager.updateWorkerProcessing(workerId, duration);
    }
  }
   ...
   */
  private async reasonWithSchemas(
    itemA: CognitiveItem,
    contextItems: CognitiveItem[]
  ): Promise<void> {
    for (const itemB of contextItems) {
      const schemas = this.schemaMatcher.find_applicable(
        itemA,
        itemB,
        this.worldModel
      );
  ...
  private async applySchema(
    ...
    try {
      ...
      for (const newItem of derived) {
        ...
        const sourceTrust =
          schemaAtom?.meta.trust_score || DEFAULT_TRUST_SCORE;
      ...
      this.schemaLearningModule.recordSchemaUsage(schema.atom_id, true, [
        itemA,
        itemB,
      ]);
    } catch (error) {
      ...
      this.schemaLearningModule.recordSchemaUsage(schema.atom_id, false, [
        itemA,
        itemB,
      ]);
    }
  ...
  private printWorkerStatistics(): void {
    ...
    for (const [
      workerId,
      stats,
    ] of this.statisticsManager.getWorkerStatistics().entries()) {
