...
export class HierarchicalGoalTreeManager implements GoalTreeManager {
  ...
  private checkParentAchievement(childId: string): void {
    ...
    const allComplete = siblings.every((siblingId) => {
      ...
      return (
        sibling &&
        (sibling.goal_status === 'achieved' || sibling.goal_status === 'failed')
      );
    });
    ...
    if (allComplete) {
      ...
      if (parent) {
        ...
        allAchieved
          ...
          : this.handleFailedParent(siblings, parentId);
      }
    }
  }
  ...
  private determineSubgoalCount(content: string): number {
    ...
    return wordCount < 5 ? 1 : wordCount < 15 ? 2 : wordCount < 30 ? 3 : 4; // Maximum of 4 subgoals
  }

  private generateSubgoalLabel(
    ...
    if (
      parentContent.toLowerCase().includes('search') ||
      parentContent.toLowerCase().includes('find')
    ) {
      const searchSteps = [
    ...
    const focus =
      focusKeywords[
        Math.floor(index / actionKeywords.length) % focusKeywords.length
      ];
