...
import { UnifiedWorldModel, WorldModelConfig, CognitiveSchema } from './UnifiedWorldModel';
...
interface PersistentWorldModelConfig extends WorldModelConfig {
...
export class RefactoredPersistentWorldModel extends UnifiedWorldModel {
  ...
  constructor(userConfig: Partial<PersistentWorldModelConfig> = {}) {
    const defaultConfig: PersistentWorldModelConfig = {
  ...
  register_schema_atom(atom: SemanticAtom): CognitiveSchema {
    const schema: CognitiveSchema = {
      atom_id: atom.id,
      apply: (a: CognitiveItem, b: CognitiveItem, worldModel: any) => {
