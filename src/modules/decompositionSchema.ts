...
export const DecompositionSchema: CognitiveSchema = {
  ...
  apply: (
    ...
    const goal =
      itemA.type === 'GOAL' && itemA.label.startsWith('Decompose:')
        ? itemA
        : itemB.type === 'GOAL' && itemB.label.startsWith('Decompose:')
          ? itemB
          : null;

    const task =
      itemA.type === 'TASK' ? itemA : itemB.type === 'TASK' ? itemB : null;
    ...
    logger.info(
      { taskId: task.id, taskLabel: task.label },
      `DecompositionSchema: Attempting to decompose task`
    );
    ...
    const relatedKnowledge = allRelated.filter(
    ...
    if (relatedKnowledge.length === 0) {
      logger.info(
        { taskId: task.id, taskLabel: task.label },
        `DecompositionSchema: No relevant knowledge found.`
      );
      return [];
    }
    ...
    if (steps.length === 0) {
      logger.warn(
        { planText },
        `DecompositionSchema: Found knowledge, but could not extract steps.`
      );
      return [];
    }
    ...
    logger.info(
      ...
      `DecompositionSchema: Found steps. Creating subtasks.`
    );
