...
export interface TaskManager {
  addTask(
    task: Omit<
      CognitiveItem,
      'id' | 'atom_id' | 'created_at' | 'updated_at' | 'stamp' | 'type'
    > & {
      type?: 'TASK';
  ...
  addEventListener(
    listener: (event: { type: string; task: CognitiveItem }) => void
  ): void;
...
export class UnifiedTaskManager implements TaskManager {
  ...
  getTasksByStatus(status: TaskStatus): CognitiveItem[] {
    return this.getAllTasks().filter(
      (task) => task.task_metadata?.status === status
    );
  }
  ...
  getTasksByPriority(
    ...
    return this.getAllTasks().filter(
      (task) => task.task_metadata?.priority_level === priority
    );
  }

  getTasksByGroupId(groupId: string): CognitiveItem[] {
    return this.getAllTasks().filter(
      (task) => task.task_metadata?.group_id === groupId
    );
  }
    ...
    if (task.task_metadata.subtasks) {
      for (const subtaskId of task.task_metadata.subtasks) {
        ...
        if (
          subtask &&
          isTask(subtask) &&
          subtask.task_metadata?.status !== 'completed'
        ) {
  ...
  addSubtask(
    ...
    if (!parentTask || !isTask(parentTask) || !parentTask.task_metadata) {
      throw new Error(
        `Parent task with ID ${parentId} not found or is missing metadata`
      );
    }
  ...
  getTaskStatistics(): {
    ...
    return {
      total: tasks.length,
      pending: tasks.filter((t) => t.task_metadata?.status === 'pending')
        .length,
      awaiting_dependencies: tasks.filter(
        (t) => t.task_metadata?.status === 'awaiting_dependencies'
      ).length,
      decomposing: tasks.filter(
        (t) => t.task_metadata?.status === 'decomposing'
      ).length,
      awaiting_subtasks: tasks.filter(
        (t) => t.task_metadata?.status === 'awaiting_subtasks'
      ).length,
      ready_for_execution: tasks.filter(
        (t) => t.task_metadata?.status === 'ready_for_execution'
      ).length,
      completed: tasks.filter((t) => t.task_metadata?.status === 'completed')
        .length,
      failed: tasks.filter((t) => t.task_metadata?.status === 'failed').length,
      deferred: tasks.filter((t) => t.task_metadata?.status === 'deferred')
        .length,
    };
  }
  ...
  private loadTasksFromWorldModel(): void {
    ...
    for (const task of tasks) {
      ...
      if (
        task.task_metadata &&
        !terminalStates.includes(task.task_metadata.status)
      ) {
  ...
  private _createAndStoreTask(
    ...
    const priorityLevel =
      taskData.task_metadata?.priority_level || TASK_PRIORITY_LEVELS.MEDIUM;
  ...
  private mapPriorityLevelToValue(
    priority_level:
      | 'low'
      | 'medium'
      | 'high'
      | 'critical' = TASK_PRIORITY_LEVELS.MEDIUM
  ): number {
