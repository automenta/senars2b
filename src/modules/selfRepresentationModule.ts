...
export class SelfRepresentationModule {
  ...
  updateComponentMetrics(componentId: string, metrics: ComponentMetrics): void {
    const component = this.architecture.components.find(
      (c) => c.id === componentId
    );
    if (component) {
      ...
      if (atom) {
        ...
        if (
          ...
          atom.content.type === 'system_component'
        ) {
  ...
  identifyCapabilityGaps(): CognitiveItem[] {
    ...
    for (const capability of desiredCapabilities) {
      const componentsWithCapability =
        this.findComponentsWithCapability(capability);
  ...
  private initializeSystemModel(): void {
    ...
    this.addComponent({
      ...
      dependencies: ['worldModel'],
      capabilities: ['goal_execution', 'result_feedback', 'action_planning'],
    });
    ...
    this.addRelationship({
      ...
      type: 'interaction',
      description:
        'Core interacts with belief revision for knowledge consistency',
    });
    ...
    this.addConstraint({
      id: 'concurrency',
      description:
        'System must support concurrent access to agenda and world model',
    ...
    this.addConstraint({
      id: 'performance',
      description:
        'System must maintain low latency for cognitive cycle execution',
