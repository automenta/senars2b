...
export const HistoryRecordingSchema: CognitiveSchema = {
  ...
  apply: (
    ...
    const eventItem =
      itemA.type === 'EVENT' && itemA.label === 'BeliefUpdated'
        ? itemA
        : itemB.type === 'EVENT' && itemB.label === 'BeliefUpdated'
          ? itemB
          : null;
    ...
    if (!itemId || !oldTruth || !newTruth) {
      logger.warn(
        ...
        'HistoryRecordingSchema: Invalid payload for BeliefUpdated event.'
      );
      return [];
    }
...
export const HistoryAnalysisSchema: CognitiveSchema = {
  ...
  apply: (
    ...
    const belief =
      itemA.type === 'BELIEF' &&
      !itemA.meta?.isHistory &&
      !itemA.meta?.analysisOf
        ? itemA
        : itemB.type === 'BELIEF' &&
            !itemB.meta?.isHistory &&
            !itemB.meta?.analysisOf
          ? itemB
          : null;
    ...
    const analyzeStability = (hist: CognitiveItem[]): CognitiveItem | null => {
      ...
      if (Date.now() - lastChange > STABILITY_THRESHOLD_MS) {
        return createMetaBelief(
          'stability',
          `Belief ${belief.id.substring(0, 8)}... is stable.`
        );
      }
      return null;
    };
    ...
    const analyzeTrend = (hist: CognitiveItem[]): CognitiveItem | null => {
      ...
      const points = hist
        .slice()
        .reverse()
        .map((h, i) => ({ x: i, y: h.truth!.confidence }));
      ...
      if (slope > TREND_THRESHOLD) {
        return createMetaBelief(
          'trend_increasing',
          `Confidence in belief ${belief.id.substring(0, 8)}... is increasing.`
        );
      }
      if (slope < -TREND_THRESHOLD) {
        return createMetaBelief(
          'trend_decreasing',
          `Confidence in belief ${belief.id.substring(0, 8)}... is decreasing.`
        );
      }
      return null;
    };

    const analyzeOscillation = (
      hist: CognitiveItem[]
    ): CognitiveItem | null => {
      ...
      if (crossings >= OSCILLATION_THRESHOLD) {
        return createMetaBelief(
          'oscillation',
          `Belief ${belief.id.substring(0, 8)}... is oscillating/contested.`
        );
      }
      return null;
    };
    ...
    const existingMetaBeliefs = (worldModel as any).query_by_meta(
      'analysisOf',
      belief.id
    );
    const existingInsights = new Set(
      existingMetaBeliefs.map((b: CognitiveItem) => b.meta?.insightType)
    );
