...
export class SchemaLearningModule {
  ...
  constructor(
    ...
    learningThreshold: number = 0.7,
    minApplications: number = 5
  ) {
  ...
  learnNewSchemas(): string[] {
    ...
    for (const [schemaId, record] of this.schemaUsageHistory.entries()) {
      ...
      if (
        record.applications >= this.minApplications &&
  ...
  getStatistics(): any {
    return {
      ...
      totalApplications: Array.from(this.schemaUsageHistory.values()).reduce(
        (sum, record) => sum + record.applications,
        0
      ),
      successfulGeneralizations: Array.from(
        this.schemaUsageHistory.values()
      ).filter(
        (record) =>
          record.applications >= this.minApplications &&
          record.successes / record.applications >= this.learningThreshold
  ...
  private extractRelationships(items: CognitiveItem[]): any {
    ...
    if (items.length >= 2) {
      return {
        ...
        goalRelationship:
          items.some((item) => item.type === 'GOAL') &&
          items.some((item) => item.type === 'BELIEF'),
      };
    }
    return {};
  }
   ...
   */
  private generalizePattern(
    originalSchemaId: string,
    pattern: SchemaPattern
  ): { atom: SemanticAtom } | null {
    try {
   ...
   */
  private applyGeneralizedSchema(
    a: CognitiveItem,
    b: CognitiveItem,
    pattern: SchemaPattern
  ): CognitiveItem[] {
