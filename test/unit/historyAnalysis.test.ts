...
describe('HistoryAnalysisSchema', () => {
  ...
  it('should not generate an insight if history is too short', () => {
    const historyItem = createHistoryItem(
      baseBelief.id,
      { frequency: 1.0, confidence: 0.6 },
      Date.now() - 10000
    );
    worldModel.add_item(historyItem);

    const insights = HistoryAnalysisSchema.apply(
      baseBelief,
      {} as CognitiveItem,
      worldModel
    );
  ...
  it('should generate a stability insight for a belief that has not changed in a long time', () => {
    ...
    worldModel.add_item(
      createHistoryItem(baseBelief.id, stableTruth, now - 2 * ONE_HOUR)
    );
    worldModel.add_item(
      createHistoryItem(baseBelief.id, stableTruth, now - 1.5 * ONE_HOUR)
    );
    worldModel.add_item(
      createHistoryItem(baseBelief.id, stableTruth, now - 1.2 * ONE_HOUR)
    );
  ...
  it('should not create duplicate meta-beliefs', () => {
    ...
    const firstPassInsights = HistoryAnalysisSchema.apply(
      baseBelief,
      {} as CognitiveItem,
      worldModel
    );
    ...
    const secondPassInsights = HistoryAnalysisSchema.apply(
      baseBelief,
      {} as CognitiveItem,
      worldModel
    );
