...
import { HistoryAnalysisSchema } from '../../src/modules/systemSchemas';
import { CognitiveItemFactory } from '../../src/modules/cognitiveItemFactory';
import { CognitiveItem, TruthValue, WorldModel } from '../../src/interfaces/types';
...
describe('HistoryAnalysisSchema', () => {
    ...
    const createHistoryItem = (beliefId: string, truth: TruthValue, timestamp: number): CognitiveItem => {
        const historyAtom = {
            ...
            embedding: [],
            meta: { type: 'Observation' as const, source: 'system_history_module', timestamp: new Date(timestamp).toISOString(), trust_score: 1.0 }
        };
        (worldModel as PersistentWorldModel).add_atom(historyAtom);

        const historicalBelief = CognitiveItemFactory.createBelief(historyAtom.id, truth, { priority: 0.1, durability: 0.9 });
    ...
    it('should generate an oscillation insight for a belief with contested frequency', () => {
        ...
        worldModel.add_item(createHistoryItem(baseBelief.id, { frequency: 0.8, confidence: constantConfidence }, now - 40000));
        worldModel.add_item(createHistoryItem(baseBelief.id, { frequency: 0.2, confidence: constantConfidence }, now - 30000));
        worldModel.add_item(createHistoryItem(baseBelief.id, { frequency: 0.9, confidence: constantConfidence }, now - 20000));
        worldModel.add_item(createHistoryItem(baseBelief.id, { frequency: 0.1, confidence: constantConfidence }, now - 10000));
    ...
    it('should not generate duplicate insights', () => {
        const now = Date.now();
        worldModel.add_item(createHistoryItem(baseBelief.id, { frequency: 1.0, confidence: 0.5 }, now - 30000));
        worldModel.add_item(createHistoryItem(baseBelief.id, { frequency: 1.0, confidence: 0.6 }, now - 20000));
        worldModel.add_item(createHistoryItem(baseBelief.id, { frequency: 1.0, confidence: 0.7 }, now - 10000));
