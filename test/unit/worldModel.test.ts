...
describe('PersistentWorldModel', () => {
  ...
  beforeEach(() => {
    ...
    const historySchemaAtom: SemanticAtom = {
      ...
      content: {
        type: 'schema',
        name: 'HistoryRecordingSchema',
        apply: HistoryRecordingSchema.apply,
      },
      embedding: [],
  ...
  describe('revise_belief', () => {
    ...
    it('should revise an existing belief and generate a BeliefUpdated event', () => {
      ...
      expect(event!.payload!.newTruth.confidence).toBe(
        revisedItem!.truth!.confidence
      );
    });
  });
  ...
  describe('History Tracking', () => {
    it('should create a historical belief item after a belief is revised', () => {
      ...
      const historicalBeliefs = HistoryRecordingSchema.apply(
        event!,
        {} as any,
        worldModel
      );
    ...
    it('should retrieve multiple historical records for an item', async () => {
      ...
      const item1 = createBeliefItem({
        id: itemId,
        truth: { frequency: 0.1, confidence: 0.2 },
      });
      ...
      const [hist1] = HistoryRecordingSchema.apply(
        event1!,
        {} as any,
        worldModel
      );
      ...
      const [hist2] = HistoryRecordingSchema.apply(
        event2!,
        {} as any,
        worldModel
      );
      ...
      expect(history[0].stamp.timestamp).toBeGreaterThan(
        history[1].stamp.timestamp
      );
    });
  });
});
