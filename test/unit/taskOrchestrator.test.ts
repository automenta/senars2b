...
describe('TaskOrchestrator', () => {
  ...
  it('should transition status from pending to awaiting_dependencies', () => {
    ...
    expect(result.updatedTask.task_metadata!.status).toBe(
  ...
  it('should remain in awaiting_subtasks if subtasks are not done', () => {
    ...
    const subtask1 = createTaskItem({
      id: 'sub1',
      task_metadata: { status: 'completed', priority_level: 'medium' },
    });
    const subtask2 = createTaskItem({
      id: 'sub2',
      task_metadata: { status: 'pending', priority_level: 'medium' },
    });
