...
const createValidTask = (
  ...
  metadataOverrides: Partial<TaskMetadata> = {}
): CognitiveItem => {
...
describe('TaskValidator', () => {
  describe('validateTask', () => {
    ...
    it('should return false for an invalid priority level', () => {
      const task = createValidTask(
        {},
        { priority_level: 'invalid_priority' as any }
      );
      expect(TaskValidator.validateTask(task)).toBe(false);
    });
  ...
  describe('normalizeTask', () => {
    ...
    it('should not overwrite existing valid data', () => {
      const task = createValidTask(
        {},
        { status: 'completed', priority_level: 'high', subtasks: ['subtask-1'] }
      );
      const normalized = TaskValidator.normalizeTask(task);
    ...
    it('should throw an error for fields with wrong types during normalization', () => {
      const taskWithInvalidTags = createValidTask(
        {},
        { tags: 'not-an-array' as any }
      );
      expect(() => TaskValidator.normalizeTask(taskWithInvalidTags)).toThrow();

      const taskWithInvalidContext = createValidTask(
        {},
        { context: 'not-an-object' as any }
      );
      expect(() =>
        TaskValidator.normalizeTask(taskWithInvalidContext)
      ).toThrow();
    });
