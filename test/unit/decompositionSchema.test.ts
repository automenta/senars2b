...
describe('DecompositionSchema', () => {
  ...
  it('should decompose a task into subtasks based on knowledge from the world model', () => {
    ...
    // 2. Execute
    const resultingSubtasks = DecompositionSchema.apply(
      decompositionGoal,
      taskToDecompose,
      mockWorldModel
    );
    ...
    expect(mockWorldModel.get_atom).toHaveBeenCalledWith(
      taskToDecompose.atom_id
    );
    // It should have performed a semantic query using the task's embedding
    expect(mockWorldModel.query_by_semantic).toHaveBeenCalledWith(
      taskAtom.embedding,
      3
    );
    ...
    expect(resultingSubtasks[0].task_metadata!.parent_id).toBe(
      taskToDecompose.id
    );

    // Check that they are chained in a dependency
    expect(resultingSubtasks[1].task_metadata!.dependencies).toEqual([
      resultingSubtasks[0].id,
    ]);
    expect(resultingSubtasks[2].task_metadata!.dependencies).toEqual([
      resultingSubtasks[1].id,
    ]);
  });
  ...
  it('should return an empty array if no relevant knowledge is found', () => {
    ...
    const result = DecompositionSchema.apply(
      decompositionGoal,
      taskToDecompose,
      mockWorldModel
    );
