...
describe('NARS Logic Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockEmbeddingService.generateEmbedding.mockImplementation(
      async (text: string) => {
        ...
        return Array(384)
          .fill(0)
          .map((_, i) => Math.abs(Math.sin(hash + i)) * 0.5 + 0.25);
      }
    );
  ...
  describe('Structural Reasoning', () => {
    it('should perform inheritance reasoning (Bird -> Animal)', async () => {
      ...
      const birdAtom = {
        ...
        embedding:
          await mockEmbeddingService.generateEmbedding('Birds are animals'),
      ...
      const tweetyAtom = {
        ...
        embedding:
          await mockEmbeddingService.generateEmbedding('Tweety is a bird'),
        creationTime: Date.now(),
        lastAccessTime: Date.now(),
        meta: { domain: 'biology' },
      };
      ...
      const foundBirdAnimalBelief = items.find(
        (item) =>
          item.type === 'BELIEF' && item.label?.includes('Birds are animals')
      );

      const foundTweetyBirdBelief = items.find(
        (item) =>
          item.type === 'BELIEF' && item.label?.includes('Tweety is a bird')
      );
    ...
    it('should perform similarity reasoning', async () => {
      ...
      const similarityAtom = {
        ...
        embedding: await mockEmbeddingService.generateEmbedding(
          'Cars and trucks are similar vehicles'
        ),
        creationTime: Date.now(),
        lastAccessTime: Date.now(),
      ...
      const wheelAtom = {
        ...
        embedding:
          await mockEmbeddingService.generateEmbedding('Cars have wheels'),
        creationTime: Date.now(),
        lastAccessTime: Date.now(),
        meta: { domain: 'transportation' },
      };
      ...
      const items = worldModel.getAllItems();
      const foundSimilarityBelief = items.find(
        (item) =>
          item.type === 'BELIEF' &&
          item.label?.includes('Cars and trucks are similar')
      );

      const foundWheelBelief = items.find(
        (item) =>
          item.type === 'BELIEF' && item.label?.includes('Cars have wheels')
      );
  ...
  describe('Procedural Reasoning', () => {
    it('should perform means-ends reasoning', async () => {
      ...
      const goalAtom = {
        ...
        embedding:
          await mockEmbeddingService.generateEmbedding('Open the door'),
        creationTime: Date.now(),
        lastAccessTime: Date.now(),
      ...
      const procedureAtom = {
        ...
        embedding: await mockEmbeddingService.generateEmbedding(
          'To open a door, one needs a key'
        ),
        creationTime: Date.now(),
        lastAccessTime: Date.now(),
        meta: { domain: 'household' },
      ...
      const goal = CognitiveItemFactory.createGoal(goalAtom.id, {
        priority: 0.9,
        durability: 0.8,
      });
      ...
      const items = worldModel.getAllItems();
      const foundGoal = items.find(
        (item) => item.type === 'GOAL' && item.label?.includes('Open the door')
      );

      const foundProcedure = items.find(
        (item) =>
          item.type === 'BELIEF' && item.label?.includes('To open a door')
      );
    ...
    it('should perform conditional reasoning (If-Then)', async () => {
      ...
      const observationAtom = {
        ...
        embedding:
          await mockEmbeddingService.generateEmbedding('It is raining'),
        creationTime: Date.now(),
        lastAccessTime: Date.now(),
        meta: { domain: 'weather' },
      };
      ...
      // Verify beliefs were added
      const items = worldModel.getAllItems();
      const foundConditional = items.find(
        (item) => item.type === 'BELIEF' && item.label?.includes('If it rains')
      );

      const foundObservation = items.find(
        (item) =>
          item.type === 'BELIEF' && item.label?.includes('It is raining')
      );
  ...
  describe('Reasoning with Negations', () => {
    it('should handle negative evidence', async () => {
      ...
      const contradictingAtom = {
        ...
        embedding:
          await mockEmbeddingService.generateEmbedding('This swan is black'),
        creationTime: Date.now(),
        lastAccessTime: Date.now(),
        meta: { domain: 'zoology' },
      };
      ...
      // Verify beliefs were added
      const items = worldModel.getAllItems();
      const foundInitialBelief = items.find(
        (item) =>
          item.type === 'BELIEF' && item.label?.includes('All swans are white')
      );

      const foundContradictingEvidence = items.find(
        (item) =>
          item.type === 'BELIEF' && item.label?.includes('This swan is black')
      );
    ...
    it('should perform contrapositive reasoning', async () => {
      ...
      const foundConditional = items.find(
        (item) =>
          item.type === 'BELIEF' && item.label?.includes('If it is a bird')
      );

      const foundNegativeEvidence = items.find(
        (item) =>
          item.type === 'BELIEF' &&
  ...
  describe('Compatibility with LM Text', () => {
    it('should process natural language inputs', async () => {
      ...
      const items = worldModel.getAllItems();
      const foundNlBelief = items.find(
        (item) => item.type === 'BELIEF' && item.label === nlInput
      );
    ...
    it('should handle uncertain LM outputs', async () => {
      ...
      const uncertainAtom = {
        ...
        embedding:
          await mockEmbeddingService.generateEmbedding(uncertainOutput),
        creationTime: Date.now(),
        lastAccessTime: Date.now(),
      ...
      const items = worldModel.getAllItems();
      const foundUncertainBelief = items.find(
        (item) => item.type === 'BELIEF' && item.label === uncertainOutput
      );
  ...
  describe('Diverse Application Domains', () => {
    it('should handle medical domain reasoning', async () => {
      ...
      const items = worldModel.getAllItems();
      const foundSymptomBelief = items.find(
        (item) =>
          item.type === 'BELIEF' && item.label?.includes('Fever is a symptom')
      );

      const foundPatientBelief = items.find(
        (item) =>
          item.type === 'BELIEF' && item.label?.includes('Patient has a fever')
      );
    ...
    it('should handle scientific domain reasoning', async () => {
      ...
      const items = worldModel.getAllItems();
      const foundBoilingPointBelief = items.find(
        (item) =>
          item.type === 'BELIEF' &&
          item.label?.includes('Water boils at 100 degrees')
      );

      const foundAltitudeBelief = items.find(
        (item) =>
          item.type === 'BELIEF' &&
    ...
    it('should handle business domain reasoning', async () => {
      ...
      const satisfactionAtom = {
        ...
        embedding: await mockEmbeddingService.generateEmbedding(
          'High customer satisfaction leads to repeat purchases'
        ),
        creationTime: Date.now(),
        lastAccessTime: Date.now(),
      ...
      const scoreAtom = {
        ...
        embedding: await mockEmbeddingService.generateEmbedding(
          'Our customer satisfaction score is 4.5 out of 5'
        ),
        creationTime: Date.now(),
        lastAccessTime: Date.now(),
      ...
      const items = worldModel.getAllItems();
      const foundSatisfactionBelief = items.find(
        (item) =>
          item.type === 'BELIEF' &&
          item.label?.includes('High customer satisfaction')
      );

      const foundScoreBelief = items.find(
        (item) =>
          item.type === 'BELIEF' &&
